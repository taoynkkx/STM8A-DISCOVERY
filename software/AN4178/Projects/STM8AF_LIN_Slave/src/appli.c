...
/* if this STM8AF board is the LIN slave */
    ...
    else
    {
      StartupCompleted = TRUE;
			...
			if (!ErrorState)
			{
				...
				if (READ_SLAVE_LD4_GREEN_LED) LD4_GREEN_LED_ON;
				else LD4_GREEN_LED_OFF;
				if (READ_SLAVE_LD5_GREEN_LED) LD5_GREEN_LED_ON;
				else LD5_GREEN_LED_OFF;
				if (READ_SLAVE_LD6_GREEN_LED) LD6_GREEN_LED_ON;
				else LD6_GREEN_LED_OFF;
				if (READ_SLAVE_LD7_GREEN_LED) LD7_GREEN_LED_ON;
...
void IncrementStatus(void)
{
  ...
  {
    BarStatus++;
    UpdateBarStatus(); /* LIN slave demo  */
  }
}
...
/**
  ...
  * @param  Ledstatus status to be displayed wth LEDs
  * @retval None
  */
